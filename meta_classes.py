...
def _execute(self, verb, params):
    """ Wrapped by the generated action method (like _get, _post ... etc). It is responsible for the actual formatting of the endpoint params, and passing values.
...
def _format(self, json_body=False, **format_args):
    ...
    json_body -- boolean, if set to True all the parameters will be sent as JSON, otherweise the paremeters will be sent as query string.
...
class RestModelMeta(type):
    ...
    def __new__(cls, clsname, bases, clsdict):
        ...
        if clsname != "RestModel":
            ...
            verbs = [x for x,y in clsdict['Meta'].__dict__.items() if type(y) == str and not x.startswith("__")]  # dynamically extracts the HTTP endpoints defined in the inner Meta class.

            method_definition = """
def _{0}(self, **params):
    return self._execute('{0}', params)
            """  # for each endpoint define a wrapper method called _verb_name. This method will be passed the parameters to be sent in the HTTP query.
            for verb in verbs:
